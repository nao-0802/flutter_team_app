...
class _MakeAlarmPageState extends State<MakeAlarmPage> {
			...
			body: ListView(
				...
				children: [
					...
					child: Column(
						crossAxisAlignment: CrossAxisAlignment.start,
						children: [
						const Text("曜日（任意）", style: TextStyle(fontWeight: FontWeight.bold)),
						const SizedBox(height: 8),
						Wrap(
							spacing: 8,
							children: days.map((d) {
	...
	Future<void> _saveAlarm() async {
		...
		ScaffoldMessenger.of(
			context,
		).showSnackBar(const SnackBar(content: Text("アラーム音を選択してください")));
		return;
		}
		...
		final nextDateTime = calclulateNextAlarmDateTime(
		selectedTime!,
		selectedDays,
		);
		await Alarm.set(
		...
		ScaffoldMessenger.of(
		context,
		).showSnackBar(const SnackBar(content: Text("アラームが保存されました")));
...
tz.TZDateTime nextWeekdayTime(int weekday, TimeOfDay time) {
...
DateTime calclulateNextAlarmDateTime(TimeOfDay time, List<String> days) {
  final now = DateTime.now();
